...
class LMConfig(PretrainedConfig):
    """ Minimind Language Model Configuration

    继承自transformers.PretrainedConfig，用于配置 Minimind 语言模型
    """
    ...
    def __init__(
            self,
            dim: int = 512,          # 模型维度，默认为 512  # TODO dim是不是词编码（embedding）的维度?
            n_layers: int = 8,       # Transformer 层数，默认为 8
            n_heads: int = 16,       # 注意力头数，默认为 16  # TODO n_heads 和 n_kv_heads 是什么关系？
            n_kv_heads: int = 8,     # KV 头数，默认为 8
            vocab_size: int = 6400,  # 词汇表大小，默认为 6400
            hidden_dim: int = None,  # 隐藏层维度，默认为 None  # TODO 什么是隐藏层？哪里会用到
            multiple_of: int = 64,   # 隐藏层维度的倍数，默认为 64  # TODO multiple_of 是什么？有什么用？
            norm_eps: float = 1e-5,  # 归一化层的 epsilon 值，默认为 1e-5  # TODO norm_eps 是什么？学习率？
            max_seq_len: int = 512,  # 最大序列长度，默认为 512
            dropout: float = 0.0,    # Dropout 概率，默认为 0.0
            flash_attn: bool = True, # 是否使用 Flash Attention，默认为 True  # TODO Flash Attention 是什么？
            ####################################################
